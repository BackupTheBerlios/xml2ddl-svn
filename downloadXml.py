...
class OracleDownloader:
    ...
    def getColumnComment(self, strTableName, strColumnName):
        """ Returns the comment as a string """
        strSql = "SELECT COMMENTS from  ALL_COL_COMMENTS WHERE TABLE_NAME = :TABLENAME AND COLUMN_NAME = :COLUMN"
        self.cursor.execute(strSql, { 'TABLENAME' : strTableName, 'COLUMN' : strColumnName })
        rows = self.cursor.fetchall()
        if rows:
            return rows[0][0]
    ...
    def getTableRelations(self, strTableName):
        """ Returns 
            ...
            or []
        """
        strSql = """SELECT CONSTRAINT_NAME, COLUMN_NAME, TABLE_NAME, pcon.confkey, pcon.confupdtype, pcon.confdeltype
            FROM pg_constraint pcon, pg_class pcla, pg_class pcla2
            WHERE pcla.relname = %s
            """
        self._tableInfo("ALL_CONSTRAINTS")
        self.cursor.execute(strSql, { 'tablename' : strTableName })
        rows = self.cursor.fetchall()
    ...
    def getViews(self):
        ...
        strQuery =  """SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES 
            ...
            TABLE_NAME NOT LIKE 'pg_%' AND TABLE_TYPE = 'VIEW'"""
        self.cursor.execute(strQuery)
        rows = self.cursor.fetchall()
        if rows:
            return [x[0] for x in rows]
